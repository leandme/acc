...
const MALE_BUCKETS: Bucket[] = [
  ...
  { label: "Fit", min: 14, max: 18, color: "#fde047" },
  { label: "Average", min: 18, max: 25, color: "#f59e0b" },
  { label: "High", min: 25, max: 32, color: "#f97316" },
  { label: "Very High", min: 32, max: 100, color: "#ef4444" },
];

const FEMALE_BUCKETS: Bucket[] = [
  { label: "Essential", min: 0, max: 14, color: "#3b82f6" },
  { label: "Athletic", min: 14, max: 21, color: "#22c55e" },
  { label: "Fit", min: 21, max: 28, color: "#fde047" },
  { label: "Average", min: 28, max: 35, color: "#f59e0b" },
  { label: "High", min: 35, max: 42, color: "#f97316" },
  { label: "Very High", min: 42, max: 100, color: "#ef4444" },
];
...
function midpoint(b: Bucket) {
  return (b.min + b.max) / 2;
}

export default function BodyFatInterpretationBar({
  bf,
  sex,
  min = 5,
  max = 45,
  ...
  const band = useMemo(() => bucketFor(sex, bf), [sex, bf]);
      ...
      <div className="w-full px-6 pt-6 pb-5">
        <div className="relative">
          ...
          <div
            ...
            aria-label={`Body fat marker snapped to ${band.label} midpoint`}
            title={band.label}
          >
            <div
              style={{
