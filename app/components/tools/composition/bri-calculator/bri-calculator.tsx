...
import BRIInterpretationBar from "./bri-interpretation-bar";
import { findBRIRange } from "./bri-ranges";
...
function calculateBRI(waistCm: number, heightCm: number) {
  if (waistCm <= 0 || heightCm <= 0) return null;

  const numerator = Math.pow(waistCm / (2 * Math.PI), 2);
  const denominator = Math.pow(0.5 * heightCm, 2);
...
function Gauge({
  ...
  const pct = clamp((value - 1) / (12 - 1), 0, 1);
...
export default function BRICalculator({ onChange }: Props) {
  ...
  const briRaw = useMemo(() => calculateBRI(waistCm, heightCm), [waistCm, heightCm]);

  const bri = useMemo(() => {
    if (briRaw == null || !Number.isFinite(briRaw)) return null;
    return round(clamp(briRaw, 0, 20), 2);
  }, [briRaw]);
  ...
  const category = useMemo(() => {
    ...
    return findBRIRange(bri);
  }, [bri]);

  useEffect(() => {
    if (bri != null && waistToHeightRatio != null && category != null) {
      onChange?.({ bri, waistToHeightRatio, category: category.label });
    }
  ...
  const displayWaist =
    units === "metric" ? `${round(waistCm, 1).toFixed(1)} cm` : `${round(cmToIn(waistCm), 1).toFixed(1)} in`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px]">
            ...
            {units === "imperial" ? (
              <>
                ...
                <SliderRow
                  ...
                  helper="Measure at navel level after a normal exhale."
                />
              </>
            ) : (
              <>
          ...
          <div className="bg-white border-l border-black/5 min-w-0">
            <div className="p-8 min-h-[420px] flex flex-col items-center justify-center text-center">
              {bri != null && category != null ? (
                <>
                  <p className="text-xs uppercase tracking-[0.16em] text-gray-500">Body Roundness Index</p>
                  <div className="mt-2">
                    <Gauge value={bri} label="BRI" rimColor={category.color} />
                  </div>
                  ...
                  <div className="mt-6 w-full grid grid-cols-2 gap-3 text-left">
                    <div className="rounded-xl bg-base-200/60 p-3">
                      <div className="text-xs text-gray-600">BRI</div>
                      <div className="text-lg font-semibold text-gray-900">{bri.toFixed(2)}</div>
                    </div>
                    <div className="rounded-xl bg-base-200/60 p-3">
                      <div className="text-xs text-gray-600">Waist / Height</div>
                      <div className="text-lg font-semibold text-gray-900">
                        {waistToHeightRatio != null ? waistToHeightRatio.toFixed(3) : "-"}
                      </div>
                    </div>
                    <div className="rounded-xl bg-base-200/60 p-3">
                    ...
                    <div className="rounded-xl bg-base-200/60 p-3">
                      ...
                      <div className="text-lg font-semibold text-gray-900">{displayWaist}</div>
                    </div>
                  </div>
                </>
              ) : (
                <p className="text-sm text-gray-500">Enter valid measurements to calculate BRI.</p>
              )}
            </div>
          </div>
        </div>

        <BRIInterpretationBar bri={bri ?? 0} />
      </div>
    </div>
  );
}
