...
import { findBRIRange, getBRIRanges, rangeMidpoint } from "./bri-ranges";

function clamp(n: number, min: number, max: number) {
  return Math.max(min, Math.min(max, n));
}

export default function BRIInterpretationBar({
  bri,
  min = 1,
  max = 12,
}: {
  ...
  const ranges = useMemo(() => getBRIRanges(), []);
  const band = useMemo(() => findBRIRange(bri), [bri]);
      ...
      <div className="w-full px-6 pt-6 pb-5">
        <div className="relative">
          ...
          <div
            ...
            aria-label={`BRI marker snapped to ${band.label} midpoint`}
            title={band.label}
          >
            <div
              style={{
