...
type Props = {
  basePath?: "/estimate" | "/body-shape-analyzer" | "/calorie-estimator" | "/height-estimator";
  buttonLabel?: string;
};
...
export default function EstimateDropZone({
  ...
  const handlePasteUrlClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation(); // ✅ prevents triggering the dropzone click (file picker)

    const url = window.prompt("Paste an image URL (must end with .jpg/.png/.webp etc):");
    if (!url) return;
