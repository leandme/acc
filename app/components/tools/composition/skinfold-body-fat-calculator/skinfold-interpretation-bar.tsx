...
export default function SkinfoldInterpretationBar({
  bodyFatPct,
  sex,
  min = 5,
  max = 45,
  ...
  const ranges = useMemo(() => getSkinfoldRanges(sex), [sex]);
  const band = useMemo(() => findSkinfoldRange(sex, bodyFatPct), [sex, bodyFatPct]);
      ...
      <div className="w-full px-6 pt-6 pb-5">
        <div className="relative">
          ...
          <div
            ...
            aria-label={`Skinfold body fat marker snapped to ${band.label} midpoint`}
            title={band.label}
          >
            <div
              style={{
        ...
        <div className="mt-2 grid grid-cols-6 text-[11px] text-gray-500">
          {ranges.map((range) => (
            <span key={range.key} className="text-center">
              {range.label}
            </span>
          ))}
        </div>
