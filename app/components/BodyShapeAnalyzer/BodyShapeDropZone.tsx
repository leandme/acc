...
export default function BodyShapeDropZone() {
  ...
  const go = (url: string) => {
    router.push(`/body-shape-analyzer?imageUrl=${encodeURIComponent(url)}`);
  };
  ...
  const handlePasteUrlClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();

    const url = window.prompt(
      "Paste an image URL (must be http(s):// and point to an image):"
    );
    if (!url) return;
    ...
    if (!/^https?:\/\//i.test(cleaned)) {
      showErrorToast("Please paste a valid URL starting with http:// or https://");
      return;
    }
