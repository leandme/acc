...
function normalizeShapeKey(input: unknown): BodyShapeKey {
  ...
  if (
    raw.includes("pear") ||
    (raw.includes("triangle") && !raw.includes("inverted"))
  ) {
    return "pear";
  }
...
function normalizeResult(raw: any): BodyShapeAnalysisResult {
  ...
  const rationale =
    raw?.analysis?.shape_rationale ??
    raw?.shape_rationale ??
    raw?.rationale ??
    null;

  const proportionNotes = asStringArray(
    raw?.analysis?.proportion_notes ?? raw?.proportion_notes
  );
  ...
  const alternatives = asStringArray(
    raw?.analysis?.alternative_shapes ?? raw?.alternative_shapes
  );
      ...
      try {
        ...
        for (let i = 0; i < 240; i++) {
          ...
          const statusRes = await fetch(
            `/api/body-shape/status?getUrl=${encodeURIComponent(getUrl)}`,
